import 'package:flutter/material.dart';

const kwidth = SizedBox(width: 10);
const kwidth20 = SizedBox(width: 20);
const khight = SizedBox(height: 10);
const khight20 = SizedBox(height: 20);
const khight40 = SizedBox(height: 40);
const khight60 = SizedBox(height: 60);
const kgreycolor = Colors.grey;
const kggreencolor = Colors.green;
const kredcolor = Colors.red;
const kwhitwcolor = Colors.white;

//style

const mainHead = TextStyle(fontSize: 20, fontWeight: FontWeight.bold);
const smallText =
    TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: kgreycolor);
const smallgreenText =
    TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: kggreencolor);

const smallgrey12 = TextStyle(fontSize: 12, color: kgreycolor);
const smallHead = TextStyle(fontSize: 13, fontWeight: FontWeight.bold);
const smallTextBlack = TextStyle(fontSize: 10, fontWeight: FontWeight.bold);
const mediumHead = TextStyle(fontSize: 16, fontWeight: FontWeight.bold);
const mediumHead13 = TextStyle(fontSize: 13, fontWeight: FontWeight.bold);

//container

BoxDecoration kboxStyle = BoxDecoration(
    borderRadius: BorderRadius.circular(10), color: Colors.grey[200]);
BoxDecoration kboxStyle1 = BoxDecoration(
    borderRadius: BorderRadius.circular(10), color: Colors.grey[350]);
BoxDecoration kboxStyle2 =
    BoxDecoration(borderRadius: BorderRadius.circular(10), color: Colors.white);
